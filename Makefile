...
qnactr: targeting-easy-fast.lisp targeting-easy-slow.lisp targeting-hard-slow.lisp targeting-hard-fast.lisp addition.lisp generateQNfile.pl qnactr/header.txt qnactr/additionheader.txt qnactr/targetheader.txt qnactralways
	...
	# incentivized
	# addition only
	perl generateQNfile.pl -n $(MODEL_TRIALS) -a -o easy -i yes && mkdir -p qnactr/block1/trial0 && cp qnactr/qnactr_model.txt qnactr/block1/trial0/QN_ACTR_Model_Initialization.txt && cp qnactr/block.txt qnactr/block1/trial0/block.txt;
	perl generateQNfile.pl -n $(MODEL_TRIALS) -a -o hard -i yes && mkdir -p qnactr/block2/trial0 && cp qnactr/qnactr_model.txt qnactr/block2/trial0/QN_ACTR_Model_Initialization.txt && cp qnactr/block.txt qnactr/block2/trial0/block.txt;
	# targeting only
	perl generateQNfile.pl -n $(MODEL_TRIALS) -t -d hard -s slow -i yes && mkdir -p qnactr/block3/trial0 && cp qnactr/qnactr_model.txt qnactr/block3/trial0/QN_ACTR_Model_Initialization.txt && cp qnactr/block.txt qnactr/block3/trial0/block.txt;
	# combined
	perl generateQNfile.pl -n $(MODEL_TRIALS) -a -o easy -t -d hard -s slow -i yes && mkdir -p qnactr/block4/trial0 && cp qnactr/qnactr_model.txt qnactr/block4/trial0/QN_ACTR_Model_Initialization.txt && cp qnactr/block.txt qnactr/block4/trial0/block.txt;
	perl generateQNfile.pl -n $(MODEL_TRIALS) -a -o hard -t -d hard -s slow -i yes && mkdir -p qnactr/block5/trial0 && cp qnactr/qnactr_model.txt qnactr/block5/trial0/QN_ACTR_Model_Initialization.txt && cp qnactr/block.txt qnactr/block5/trial0/block.txt;

publish-models: qnactr
	mkdir -p $(exp_dir)/output/subject$(MODELNUM)/block{1,2,3,4,5}/trial0
